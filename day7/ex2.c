// #define NOB_IMPLEMENTATION
#include "nob.h"
#include "utils.h"

typedef enum {
    HIGH_CARD,
    ONE_PAIR,
    TWO_PAIR,
    THREE_KIND,
    FULL_HOUSE,
    FOUR_KIND,
    FIVE_KIND,
} Type;

const char cards[] = "J23456789TQKA";
#define CARDS_SIZE (sizeof(cards) - 1)

typedef struct {
    char c;
    uint8_t count;
} Token;

typedef struct {
    char cards[5];

    uint64_t bid;
    int64_t rank;
    Type type;

    Token tokens[5];
    uint8_t ntok;
} Hand;

typedef struct {
    Hand *items;
    size_t count, capacity;
} Hands;

int sort_tokens(const void *a, const void *b) { return (((Token *)b)->count - ((Token *)a)->count); }

int sort_ranks(const void *a, const void *b) {
    Hand h1 = *(Hand *)b;
    Hand h2 = *(Hand *)a;
    // printf("----------------------------\n");
    // printf("h1 -> " SV_Fmt "\nh2 ->" SV_Fmt "\n", 5, h1.cards, 5, h2.cards);

    int result = h1.type - h2.type;

    if (result == 0) {
        for (size_t i = 0; i < 5; ++i) {
            result = strchr(cards, h1.cards[i]) - strchr(cards, h2.cards[i]);
            if (result != 0)
                break;
        }
        // printf("Result: %d\n", result);
    }

    return result;
}

void addValue(char *s, char ch, size_t len) {
    for (size_t i = 0; i < len; ++i) {
        if (s[i] == 0) {
            s[i] = ch;
            return;
        }
    }
    NOB_UNREACHABLE("addValue");
}

// #define SMALL
int main(int argc, char **argv) {
    int result = 0;

    const char *input = NULL;
    if (argc > 1)
        input = argv[1];
    else {
        char *fullPath = GetFullPath(__FILE__, NULL, 0);
        if (fullPath != NULL) {
            fullPath[nob_path_name(fullPath) - fullPath] = '\0';
            if (!nob_set_current_dir(fullPath)) {
                return 1;
            }

            free(fullPath);
        }

#ifdef SMALL
        input = "small.txt";
#else
        input = "input.txt";
#endif
    }

    Nob_String_Builder sb = {};
    if (!nob_read_entire_file(input, &sb))
        nob_return_defer(1);

    Hand h                 = {};
    Hands hands            = {};
    uint64_t totalWinnings = 0;

    Nob_String_View sv = nob_sv_trim(nob_sb_to_sv(sb));
    while (sv.count) {
        Nob_String_View line = nob_sv_trim(nob_sv_chop_by_delim(&sv, '\n'));
        memcpy(h.cards, line.data, 5);
        nob_sv_chop_by_delim(&line, ' ');

        line   = nob_sv_trim(line);
        h.bid  = nob_sv_to_u64(line);
        h.rank = -1;

        nob_da_append(&hands, h);
        memset(&h, 0, sizeof(h));
    }

    nob_sb_free(sb);
    memset(&sb, 0, sizeof(sb));

    char cs[5];
    for (size_t i = 0; i < hands.count; ++i) {
        memset(cs, 0, sizeof(cs));
        h = hands.items[i];

        for (size_t j = 0; j < 5; ++j) {
            char c        = h.cards[j];
            uint8_t count = 1;
            assert(strchr(cards, c) != NULL);

            if (memchr(cs, c, 5))
                continue;

            addValue(cs, c, sizeof(cs));

            for (size_t k = j + 1; k < 5; ++k)
                if (c == h.cards[k])
                    count++;

            assert(count <= 5);
            h.tokens[j].c     = c;
            h.tokens[j].count = count;
            h.ntok++;
        }

        qsort(h.tokens, 5, sizeof(Token), sort_tokens);

        if (memchr(cs, 'J', sizeof(cs)) != NULL) {
            Token *joker = NULL;
            for (size_t j = 0; j < h.ntok; ++j) {
                if (h.tokens[j].c == 'J') {
                    joker = &h.tokens[j];
                    break;
                }
            }
            assert(joker != NULL && joker->c != 0 && joker->count != 0);
            if (joker->count < 5) {
                uint8_t index = 0;
                while (index < h.ntok && h.tokens[index].c == 'J') {
                    index++;
                }
                if (index < h.ntok) {
                    // printf("starting index: %u (%c)\n", index, h.tokens[index].c);
                    for (size_t j = index + 1; j < h.ntok; ++j) {
                        if (h.tokens[j].c == 'J')
                            continue;
                        if (h.tokens[j].count > h.tokens[index].count) {
                            index = j;
                        }
                    }
                    h.tokens[index].count += joker->count;
                    h.ntok--;
                    joker->c     = 0;
                    joker->count = 0;

                    qsort(h.tokens, 5, sizeof(Token), sort_tokens);
                }
            }
        }

        hands.items[i] = h;

        // nob_log(NOB_INFO, "cs: " SV_Fmt " ntok: %u", 5, cs, h.ntok);
    }

    for (size_t i = 0; i < hands.count; ++i) {
        h = hands.items[i];
        for (size_t j = 0; j < h.ntok; ++j) {
            printf("%c", h.tokens[j].c);
        }
        puts("");
    }

    for (size_t i = 0; i < hands.count; ++i) {
        h = hands.items[i];

        switch (h.ntok) {
        case 5:
            h.type = HIGH_CARD;
            break;
        case 4:
            h.type = ONE_PAIR;
            break;
        case 3:
            if ((h.tokens[0].count == 3) && (h.tokens[1].count == 1)) { // Three of a kind -> TTT98
                h.type = THREE_KIND;
            } else if ((h.tokens[0].count == 2) && (h.tokens[1].count == 2)) { // Two pair -> 23432
                h.type = TWO_PAIR;
            } else {
                printf("hand: " SV_Fmt "\n", 5, h.cards);
                printf("ntok: %u\n", h.ntok);
                for (uint8_t i = 0; i < h.ntok; ++i) {
                    printf("Token[%u]: '%c' = %u\n", i, h.tokens[i].c, h.tokens[i].count);
                }
                fflush(stdout);
                NOB_UNREACHABLE("switch(h.ntok) case 3");
            }
            break;
        case 2:
            if ((h.tokens[0].count == 3) && (h.tokens[1].count == 2)) { // Full House -> 23332
                h.type = FULL_HOUSE;
            } else if ((h.tokens[0].count == 4) && (h.tokens[1].count == 1)) { // Four of a kind -> AA8AA
                h.type = FOUR_KIND;
            } else {
                NOB_UNREACHABLE("switch(h.ntok) case 2");
            }
            break;
        case 1:
            h.type = FIVE_KIND;
            break;
        default:
            NOB_UNREACHABLE("switch(h.ntok)");
            break;
        }
        // h.rank = rank--;

        hands.items[i] = h;

        // printf("rank: %d\n", h.rank);
        // totalWinnings += (h.rank * h.bid);
    }

    qsort(hands.items, hands.count, sizeof(Hand), sort_ranks);
    printf("----------------------------\n");
    size_t rank = hands.count;
    for (size_t i = 0; i < hands.count; ++i) {
        h      = hands.items[i];
        h.rank = rank--;

        printf("----------------------------\n");
        printf("Cards: " SV_Fmt "\n", 5, h.cards);
        printf("Bid: %zu\n", h.bid);

        for (size_t j = 0; j < 5; ++j) {
            if (!h.tokens[j].count)
                break;
            printf("'%c' -> %d %s\n", h.tokens[j].c, h.tokens[j].count, (h.tokens[j].count > 1) ? "times" : "time");
        }

        // printf("Tokens -> " SV_Fmt " (%d)\n", (int)sizeof(cs), cs, h.ntok);
        printf("Num Tokens -> %d\n", h.ntok);
        printf("Rank -> %zd\n", h.rank);
        printf("Calc: %zu\n", (h.bid * h.rank));
        totalWinnings += (h.bid * h.rank);
    }
    printf("----------------------------\n");
    printf("Total Winnings: %zu\n", totalWinnings);

    fflush(stdout);

    nob_log(NOB_INFO, "Total Winnings: %zu", totalWinnings);

defer:
    nob_sb_free(sb);
    nob_da_free(hands);

    return result;
}